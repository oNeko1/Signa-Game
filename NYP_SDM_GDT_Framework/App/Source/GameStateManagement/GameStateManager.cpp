...
bool CGameStateManager::Update(const double dElapsedTime)
{
	...
	if (nextGameState != activeGameState)
	{
		...
		cout << "Initialising " << activeGameState->GetName() << endl;
		if (activeGameState->Init() == false)
		{
			cout << "Unable to initialise " << activeGameState->GetName() << endl;
			return false;
		}
		else
		{
			cout << "Loading " << activeGameState->GetName() << endl;
		}
	}
